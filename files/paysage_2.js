...
Mot.visibiliteCode = function(varname,visibilite){
	
    var code =  "var "+varname+" = new Object()\n"
    for(distance in visibilite){
	code += varname+"["+distance+"]=[["
...
/*
    var mots_background = Mot.background.mots()
    for(var i in mots_background){
	var mot = mots_background[i]
	code += 'var ' + mot.varname + ' = ' +mot.constructionCode()+'\n'
    }
*/
    var bg_visibilite_var_name = "VISIBILITE_BACKGROUND"
  	code += Mot.visibiliteCode(bg_visibilite_var_name,Mot.background.visibilite)
  	code += "CONFIGURATION.visibilite_background = "+bg_visibilite_var_name+"\n\n"

    if(globals.montreur.is_optimized()){
      var visibilite_var_name = "VISIBILITE"
	  code += Mot.visibiliteCode(visibilite_var_name,globals.montreur.visibilite)
	  code += "CONFIGURATION.visibilite = "+visibilite_var_name+"\n"
    }
...
//    globals.instruction.setAttributeNS(null,"visibility","hidden");
    ...
    globals.intervalId=globals.timer.setInterval(readSensor,80)
...
function Montreur(premier_mot){
    this.premier_mot = premier_mot
    this.visible = [premier_mot]
    this.visible[0].show(true)
    this.last_foreground = null
    ...
    this.adjustAllToFontSize = function(){
     ...
     if(size<=Mot.background.maxSize){
    	 this.last_foreground = this.visible.slice()
    	 for(var i in this.visible){
		    this.visible[i].show(false)
    	 }
    	 Mot.background.showFor(size)
    	 return Mot.background.hasChanged()
     }else if(this.last_foreground){
    	 this.visible = this.last_foreground
    	 for(var i in this.visible){
 		    this.visible[i].show(true)
     	 }
    	 this.last_foreground=null
    	 Mot.background.showFor(size)
    	 hasChanged = true
	...
	//distance 270 à 10/20
	   ...
	   while(globals.montreur.adjustAllToFontSize()){}
	   var changed = true
	   for(var size = globals.maxSize; size>10;size-=1){
	     if(changed){
		   var visible = []
		   var invisible = []
    ...
    this.adjustAllToFontSize_optimized = function(){
        ...
        var key = null
        var max = null
	    for(var i in visibilite){
	    	var index = parseInt(i)
	    	if(index>=size){
		        //plus petite distance supérieure à size
	    		if(!key || key>index){key=index}
	    	}
	    	if(!max || max<index ){max=index}
	    }
	    if(!key){	key=max }
    ...
    this.optimize = function(){
	  ...
	  this.adjustAllToFontSize_not_optimized = globals.montreur.adjustAllToFontSize
...
function Background(){
  ...
  for(var x = 65+0*101.5; x<globals.totalWidth-20; x+=101.5){
    for(var y = 15+0*36; y<globals.totalHeight-15; y+=36){
      ...
      pixel.isInBackground = true
      this.pixels.push(pixel)
      this.devant.push(pixel)
      if(empreinte.intersects(pixel) || nez.intersects(pixel)){
          this.coveredPixels.push(pixel)
          this.derriere.push(pixel)
      }
    }
  }
  this.visibilite[35] = [this.devant,this.empreinte]
  this.visibilite[28] = [this.derriere,this.coveredPixels]
  this.size = null
  this.lastSize = null
  
  this.mots = function(){
     return this.empreinte.concat(this.pixels)
  }
  
  this.maxSize = 0
  for(var i in this.visibilite){
		  var index = parseInt(i)
		  if(index>this.maxSize){this.maxSize= index}
  }
  
  this.isVisible = function(size){
	  return parseInt(size)<=this.maxSize
  }
  
  this.isInBackground = function(mot){
	  return mot.isInBackground
  }
  
  this.showFor = function (size){
	  var visible = this.visibleFor(size)
	  for(var i in visible[0]){
		  visible[0][i].show(true)
	  }
	  for(var i in visible[1]){
		  visible[1][i].show(false)
	  }
  }
  
  this.visibleFor = function(size){
	  ...
	  this.size = key
      return this.visibilite[key]
  }
  
  this.hasChanged = function(){
	  return this.size == this.lastSize
  }
}
...
/* globals.instruction = svghelper.createText("Appuyer 'D' pour démarrer",30,20,30)
 ...
 globals.instruction.setAttributeNS(null,"text-anchor", "end")
